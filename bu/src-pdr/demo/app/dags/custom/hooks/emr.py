...
from custom.utils.waiter import get_state, waiter
import custom.utils.subnet as subnet_verifier
...
class EmrHook(AwsBaseHook):
    ...
    def create_job_flow(self, job_flow_overrides: dict[str, Any]) -> dict[str, Any]:
        ...
        if not ("Ec2SubnetId" in config["Instances"] or "Ec2SubnetIds" in config["Instances"]):      
                ...
                raise AirflowException(f"Cluster creation failed. Please verify cluster spec and provide private Subnet:\n{pretty_json}")
        else:
            # Check Subnets
            instances = config["Instances"]
            # if instances["Ec2SubnetIds"]:
            #      subnetIds = instances["Ec2SubnetIds"]
            if instances["Ec2SubnetId"]:
                subnetIds.append(instances["Ec2SubnetId"])
            #Validate if its Private subnet
            region = subnet_verifier.detect_running_region()
            subnet_verifier.client(region_name=region)
            isPrivateSubnet = subnet_verifier.validate_private_subnet(subnetIds)
