...
resource "aws_glue_crawler" "glue_crawler" {
  ...
  name          = format("%s-%s-%s", var.resource_prefix, var.glue_crawler_name, each.key)
  database_name = format("%s-%s", var.resource_prefix, each.key)
  ...
  s3_target {
    path = format("%s://%s", "s3", each.value.bucket)
    # TODO - enable glue connector
